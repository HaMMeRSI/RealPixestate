const helloWorld = artifacts.require('RealPixestate');
...
contract('RealPixestate', (accounts) => {
	...
	it('should mint with discount', async () => {
		...
		await realPixestate.safeMint(accounts[1], getTokenId([10, 10], [19, 19]), myToken.address, { from: accounts[2] });
	...
	it('should mint with discount2', async () => {
		...
		await realPixestate.safeMint(accounts[1], getTokenId([20, 20], [29, 29]), myToken.address, { from: accounts[2] });
		assert.equal((await myToken.balanceOf(accounts[0])).toNumber(), 150);
		// await realPixestate.safeMint(accounts[0], getTokenId([45, 45], [500, 46]));
	});
	...
	it('should break', async () => {
		...
		const myToken = await MyToken.deployed();
		await realPixestate.safeMint(accounts[0], tokenId0, myToken.address, { from: accounts[1] });
		assert.equal(await realPixestate.ownerOf(tokenId0), accounts[0]);
		...
		assert.equal(await realPixestate.ownerOf(tokenId1), accounts[0]);
		assert.equal(await realPixestate.ownerOf(tokenId2), accounts[0]);
		assert.equal(await realPixestate.ownerOf(tokenId3), accounts[0]);
